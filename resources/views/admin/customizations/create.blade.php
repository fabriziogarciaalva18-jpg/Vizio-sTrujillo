                ...
                <form action="{{ route('admin.customizations.store') }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control form-control-retro" value="{{ old('name') }}" required placeholder="Ej: Grande, Chocolate, Fondant...">
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <select name="product_ids[]" class="form-select form-control-retro" multiple required style="min-height: 120px;">
                                @foreach($products as $p)
                                    <option value="{{ $p->id }}" {{ in_array($p->id, old('product_ids', [])) ? 'selected' : '' }}>
                                        {{ $p->name }}
                                    </option>
                                @endforeach
                            </select>
